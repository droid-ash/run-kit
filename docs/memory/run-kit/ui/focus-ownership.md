...
## The Operator Console's Own Focus Return

The desktop operator console does **not** participate in the focus-memory module above: it never calls `recordFocus`/`recallFocus`, never routes through the restore router, and holds its own origin ref (`restoreFocusRef` in `components/operator-omnibox.tsx`) for the ⌘J machine's focus return ([operator-console](/run-kit/ui/operator-console.md) § The ⌘J three-state machine). The reason is route coverage: the omnibox renders in the top bar on Host, Board, and tmux Server routes as well as Terminal, and the restore router resolves a `(server, windowId)` focus key that exists on only one of the four.

A local origin ref is safe only under the two invariants that give it the router's own safety property — it can act toward, but never against, a genuine user choice:

- **An origin inside the box is never recorded.** The capture runs at machine entry and keeps `document.activeElement` only when it lies outside the omnibox wrapper. A mouse entry focuses the input before `onFocus` transitions the machine, so an unguarded capture names the box itself.
- **The return is ownership-gated.** On reaching `rest` the effect takes no focus action unless the omnibox input is still `document.activeElement`. A release the user caused by focusing a terminal pane already has its owner; a focus call there is a steal.

Violating either composes them into a loop: the restore re-focuses the box, whose `onFocus` re-enters the machine, so the box holds keyboard focus against every release path including Esc, and keystrokes meant for an xterm pane land in the compose draft. The terminal is a passive party throughout — xterm's own `mousedown` focuses its helper textarea correctly on a fine pointer (the suppressors in `terminal-client.tsx` are all coarse-pointer and scroll-lock gated, § [terminal](/run-kit/ui/terminal.md)); the yield is achieved by the console declining to act, not by routing focus back to it. jsdom cannot prove any of this — its synthetic focus events never move `document.activeElement` — so the terminal-route yield is covered in Playwright (`operator-console.spec.ts`). (260906-f4s6)
...
### The operator console keeps a guarded origin ref instead of the restore router
**Decision**: the omnibox's ⌘J focus return holds its own `document.activeElement` origin ref rather than routing through `recallFocus`/`restoreFocus`, under two invariants — an origin inside the box is never recorded, and the return is skipped unless the box still owns focus (§ The Operator Console's Own Focus Return).
**Why**: the omnibox renders in the top bar on Host, Board, and tmux Server routes as well as Terminal, and the restore router's `(server, windowId)` key exists on only the Terminal route — the router has no target to resolve on three of the four. The two invariants supply the property that actually matters, which is not *where* the focus target comes from but that the mechanism can never override a newer genuine user choice.
**Rejected**: routing the console's return through `restoreFocus` — unresolvable on the majority of routes the console renders on; and an unguarded origin ref, which composes its two rules into a self-restoring loop the user cannot escape.
